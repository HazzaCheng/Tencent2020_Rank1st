...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # 这里的 input 包括了序列信息每次点击我们构造出的 embedding，以及序列信息每个特征的 word 在 BERT 词表里的 embedding
    ...
    def forward(self, dense_features, text_features, text_ids, text_masks, text_features_1, text_masks_1, labels=None):
        ...
        # text_ids shape [B, max_len_text, len(self.args.text_features)]
        # text_embedding shape [B, args.max_len_text, len(args.text_features) * args.vocab_dim_v1]
        text_embedding = self.text_embeddings(text_ids).view(text_ids.size(0), text_ids.size(1), -1)
        # text_features shape [B, args.max_len_text, args.text_dim]
        # -> [B, args.max_len_text, args.text_dim + len(args.text_features) * args.vocab_dim_v1]
        ...
        # text_masks shape [B, args.max_len_text]
        # hidden_states shape [B, args.max_len_text, args.hidden_size]
        # embed_mean shape [B, args.hidden_size]
        ...
        # text_features_1 shape [B, args.max_len_text, args.text_dim_1]
        # -> [B, args.max_len_text, args.text_dim_1 + args.hidden_size]
        ...
        # final_hidden_state shape [B, dense_features_size + args.hidden_size * 2 + 512 * 2] == [B, args.out_size]
        final_hidden_state = torch.cat(outputs, -1)
        # logits shape [B, args.num_label]
